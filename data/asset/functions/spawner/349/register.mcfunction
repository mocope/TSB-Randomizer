...
execute unless data storage randomizer: Pos unless loaded 105 94 -229 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 349
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [105, 94, -229]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:261,Weight:10},{Id:73,Weight:10}]
# 一度に召喚する数 (int)
