...
execute unless data storage randomizer: Pos unless loaded 257 106 -275 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1142
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [257, 106, -275]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [271]
# 一度に召喚する数 (int)
