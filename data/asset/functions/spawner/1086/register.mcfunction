...
execute unless data storage randomizer: Pos unless loaded 55 19 580 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1086
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [55, 19, 580]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:80,Weight:4}]
# 一度に召喚する数 (int)
