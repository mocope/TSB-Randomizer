...
execute unless data storage randomizer: Pos unless loaded -203 209 393 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1153
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-203, 209, 393]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:228,Weight:1},{Id:210,Weight:1},{Id:257,Weight:2}]
# 一度に召喚する数 (int)
    data modify storage asset:spawner SpawnCount set value 3
# 動作範囲 (int) この範囲にプレイヤーが存在するとき、Mobの召喚を開始する
    data modify storage asset:spawner SpawnRange set value 6
# 初回召喚時間 (int)
...
# この範囲にプレイヤーが存在するとき、Mobの召喚を開始する // distance <= 100
    data modify storage asset:spawner RequiredPlayerRange set value 9
