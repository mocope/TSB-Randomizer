...
execute unless data storage randomizer: Pos unless loaded -54 28 -135 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 55
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-54, 28, -135]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [7,166]
# 一度に召喚する数 (int)
