...
execute unless data storage randomizer: Pos unless loaded -435 76 86 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1004
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-435, 76, 86]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [417,418]
# 一度に召喚する数 (int)
