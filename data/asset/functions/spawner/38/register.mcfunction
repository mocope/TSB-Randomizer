...
execute unless data storage randomizer: Pos unless loaded -80 17 14 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 38
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-80, 17, 14]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:239,Weight:15}]
# 一度に召喚する数 (int)
