...
execute unless data storage randomizer: Pos unless loaded 15 42 376 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 487
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [15, 42, 376]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [53,313]
# 一度に召喚する数 (int)
