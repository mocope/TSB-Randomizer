...
execute unless data storage randomizer: Pos unless loaded 95 62 613 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1099
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [95, 62, 613]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:80,Weight:3}]
# 一度に召喚する数 (int)
