...
execute unless data storage randomizer: Pos unless loaded -460 85 48 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1023
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-460, 85, 48]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [287,260]
# 一度に召喚する数 (int)
