...
execute unless data storage randomizer: Pos unless loaded 92 161 429 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 887
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [92, 161, 429]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [156,153]
# 一度に召喚する数 (int)
