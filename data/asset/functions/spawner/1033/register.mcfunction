...
execute unless data storage randomizer: Pos unless loaded -469 104 58 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1033
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-469, 104, 58]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [418]
# 一度に召喚する数 (int)
