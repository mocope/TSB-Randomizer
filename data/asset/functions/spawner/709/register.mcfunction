...
execute unless data storage randomizer: Pos unless loaded -256 130 -174 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 709
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-256, 130, -174]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [187,182]
# 一度に召喚する数 (int)
