...
execute unless data storage randomizer: Pos unless loaded -75 18 -59 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 47
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-75, 18, -59]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:238,Weight:10},{Id:31,Weight:10}]
# 一度に召喚する数 (int)
