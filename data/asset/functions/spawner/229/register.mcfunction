...
execute unless data storage randomizer: Pos unless loaded -139 47 57 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 229
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-139, 47, 57]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [57]
# 一度に召喚する数 (int)
