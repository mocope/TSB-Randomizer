...
execute unless data storage randomizer: Pos unless loaded -82 27 226 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 124
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-82, 27, 226]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [37]
# 一度に召喚する数 (int)
