...
execute unless data storage randomizer: Pos unless loaded 455 201 -153 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 582
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [455, 201, -153]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [414,204]
# 一度に召喚する数 (int)
