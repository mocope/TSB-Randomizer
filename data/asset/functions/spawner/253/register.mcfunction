...
execute unless data storage randomizer: Pos unless loaded -149 49 -120 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 253
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-149, 49, -120]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [227]
# 一度に召喚する数 (int)
