...
execute unless data storage randomizer: Pos unless loaded 178 7 -138 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 171
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [178, 7, -138]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:49,Weight:15},{Id:259,Weight:5}]
# 一度に召喚する数 (int)
