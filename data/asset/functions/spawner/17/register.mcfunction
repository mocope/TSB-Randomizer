...
execute unless data storage randomizer: Pos unless loaded 115 22 40 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 17
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [115, 22, 40]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:77,Weight:15},{Id:58,Weight:5}]
# 一度に召喚する数 (int)
