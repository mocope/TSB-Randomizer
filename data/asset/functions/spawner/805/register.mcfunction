...
execute unless data storage randomizer: Pos unless loaded 399 22 408 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 805
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [399, 22, 408]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:84,Weight:8},{Id:369,Weight:12}]
# 一度に召喚する数 (int)
