...
execute unless data storage randomizer: Pos unless loaded 199 10 -132 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 175
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [199, 10, -132]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:259,Weight:10},{Id:241,Weight:5},{Id:242,Weight:5}]
# 一度に召喚する数 (int)
