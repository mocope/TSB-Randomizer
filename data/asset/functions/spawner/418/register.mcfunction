...
execute unless data storage randomizer: Pos unless loaded -288 64 180 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 418
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-288, 64, 180]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:47,Weight:10},{Id:257,Weight:10}]
# 一度に召喚する数 (int)
