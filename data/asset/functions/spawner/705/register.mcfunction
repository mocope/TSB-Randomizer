...
execute unless data storage randomizer: Pos unless loaded -282 133 -174 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 705
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-282, 133, -174]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [220,201]
# 一度に召喚する数 (int)
