...
execute unless data storage randomizer: Pos unless loaded -227 195 4 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1150
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-227, 195, 4]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:261,Weight:1},{Id:80,Weight:1}]
# 一度に召喚する数 (int)
