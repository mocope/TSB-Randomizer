...
execute unless data storage randomizer: Pos unless loaded -76 240 390 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 288
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-76, 240, 390]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [257,181]
# 一度に召喚する数 (int)
