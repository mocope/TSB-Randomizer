...
execute unless data storage randomizer: Pos unless loaded -57 28 -165 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 60
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-57, 28, -165]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:62,Weight:12},{Id:30,Weight:8}]
# 一度に召喚する数 (int)
