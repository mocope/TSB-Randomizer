...
execute unless data storage randomizer: Pos unless loaded 229 117 -34 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 286
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [229, 117, -34]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:253,Weight:10},{Id:139,Weight:10}]
# 一度に召喚する数 (int)
