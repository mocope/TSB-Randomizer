...
execute unless data storage randomizer: Pos unless loaded 135 60 588 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1095
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [135, 60, 588]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [330,76]
# 一度に召喚する数 (int)
