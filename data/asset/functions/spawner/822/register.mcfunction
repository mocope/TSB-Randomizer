...
execute unless data storage randomizer: Pos unless loaded 55 72 113 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 822
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [55, 72, 113]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [337,76]
# 一度に召喚する数 (int)
