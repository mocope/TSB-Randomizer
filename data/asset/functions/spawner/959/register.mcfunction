...
execute unless data storage randomizer: Pos unless loaded 76 176 -469 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 959
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [76, 176, -469]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [201,363,414]
# 一度に召喚する数 (int)
