...
execute unless data storage randomizer: Pos unless loaded 70 16 223 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 195
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [70, 16, 223]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [49]
# 一度に召喚する数 (int)
