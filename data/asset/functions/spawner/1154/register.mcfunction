...
execute unless data storage randomizer: Pos unless loaded -203 192 393 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1154
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-203, 192, 393]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:228,Weight:1},{Id:210,Weight:1}]
# 一度に召喚する数 (int)
...
# この範囲にプレイヤーが存在するとき、Mobの召喚を開始する // distance <= 100
    data modify storage asset:spawner RequiredPlayerRange set value 24
