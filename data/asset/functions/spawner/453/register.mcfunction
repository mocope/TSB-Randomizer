...
execute unless data storage randomizer: Pos unless loaded 249 138 100 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 453
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [249, 138, 100]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [415,182]
# 一度に召喚する数 (int)
