...
execute unless data storage randomizer: Pos unless loaded -149 56 -107 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 245
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-149, 56, -107]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [178]
# 一度に召喚する数 (int)
