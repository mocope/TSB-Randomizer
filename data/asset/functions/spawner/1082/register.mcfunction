...
execute unless data storage randomizer: Pos unless loaded 129 19 648 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1082
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [129, 19, 648]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:331,Weight:4},{Id:80,Weight:6}]
# 一度に召喚する数 (int)
    data modify storage asset:spawner SpawnCount set value 3
# 動作範囲 (int) この範囲にプレイヤーが存在するとき、Mobの召喚を開始する
    data modify storage asset:spawner SpawnRange set value 16
# 初回召喚時間 (int)
