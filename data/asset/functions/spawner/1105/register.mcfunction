...
execute unless data storage randomizer: Pos unless loaded 77 116 573 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1105
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [77, 116, 573]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [331,210]
# 一度に召喚する数 (int)
