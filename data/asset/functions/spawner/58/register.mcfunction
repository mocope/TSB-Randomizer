...
execute unless data storage randomizer: Pos unless loaded -60 34 -154 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 58
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-60, 34, -154]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [62]
# 一度に召喚する数 (int)
