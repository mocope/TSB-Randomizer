...
execute unless data storage randomizer: Pos unless loaded 160 10 193 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 224
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [160, 10, 193]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:206,Weight:15},{Id:19,Weight:5}]
# 一度に召喚する数 (int)
