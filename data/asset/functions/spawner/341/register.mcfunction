...
execute unless data storage randomizer: Pos unless loaded 278 94 -206 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 341
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [278, 94, -206]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [107]
# 一度に召喚する数 (int)
