...
execute unless data storage randomizer: Pos unless loaded -197 187 196 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 753
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-197, 187, 196]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [160,228]
# 一度に召喚する数 (int)
