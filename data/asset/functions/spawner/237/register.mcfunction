...
execute unless data storage randomizer: Pos unless loaded -41 2 85 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 237
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-41, 2, 85]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:57,Weight:15},{Id:259,Weight:5}]
# 一度に召喚する数 (int)
