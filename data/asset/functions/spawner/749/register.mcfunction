...
execute unless data storage randomizer: Pos unless loaded -155 199 172 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 749
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-155, 199, 172]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:160,Weight:2},{Id:228,Weight:1},{Id:424,Weight:1}]
# 一度に召喚する数 (int)
