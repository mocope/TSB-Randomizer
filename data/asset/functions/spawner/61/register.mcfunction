...
execute unless data storage randomizer: Pos unless loaded 44 21 -124 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 61
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [44, 21, -124]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:161,Weight:10},{Id:29,Weight:10}]
# 一度に召喚する数 (int)
