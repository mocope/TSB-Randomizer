...
execute unless data storage randomizer: Pos unless loaded 422 25 406 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 806
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [422, 25, 406]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:187,Weight:10},{Id:219,Weight:10}]
# 一度に召喚する数 (int)
