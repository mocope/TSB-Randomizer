...
execute unless data storage randomizer: Pos unless loaded 70 -12 40 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 96
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [70, -12, 40]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [31]
# 一度に召喚する数 (int)
