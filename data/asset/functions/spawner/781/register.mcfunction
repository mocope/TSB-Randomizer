...
execute unless data storage randomizer: Pos unless loaded -208 76 393 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 781
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-208, 76, 393]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [189,370,414]
# 一度に召喚する数 (int)
