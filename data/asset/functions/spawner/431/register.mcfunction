...
execute unless data storage randomizer: Pos unless loaded -76 166 186 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 431
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-76, 166, 186]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:74,Weight:8},{Id:235,Weight:8},{Id:245,Weight:4}]
# 一度に召喚する数 (int)
