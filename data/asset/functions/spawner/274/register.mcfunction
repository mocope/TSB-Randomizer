...
execute unless data storage randomizer: Pos unless loaded 224 149 -86 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 274
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [224, 149, -86]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [138]
# 一度に召喚する数 (int)
