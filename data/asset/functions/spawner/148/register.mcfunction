...
execute unless data storage randomizer: Pos unless loaded 158 82 37 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 148
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [158, 82, 37]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:236,Weight:5},{Id:37,Weight:15}]
# 一度に召喚する数 (int)
