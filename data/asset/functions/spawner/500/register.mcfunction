...
execute unless data storage randomizer: Pos unless loaded 40 48 349 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 500
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [40, 48, 349]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [313,139]
# 一度に召喚する数 (int)
