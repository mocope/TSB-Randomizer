...
execute unless data storage randomizer: Pos unless loaded 94 73 555 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1098
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [94, 73, 555]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:330,Weight:3},{Id:83,Weight:1}]
# 一度に召喚する数 (int)
