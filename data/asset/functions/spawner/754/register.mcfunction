...
execute unless data storage randomizer: Pos unless loaded -202 186 171 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 754
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-202, 186, 171]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:159,Weight:2},{Id:370,Weight:1},{Id:424,Weight:1}]
# 一度に召喚する数 (int)
