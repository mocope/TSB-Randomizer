...
execute unless data storage randomizer: Pos unless loaded 174 155 -195 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 597
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [174, 155, -195]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [273]
# 一度に召喚する数 (int)
