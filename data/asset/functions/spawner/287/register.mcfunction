...
execute unless data storage randomizer: Pos unless loaded 246 131 -66 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 287
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [246, 131, -66]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [415]
# 一度に召喚する数 (int)
...
# この範囲にプレイヤーが存在するとき、Mobの召喚を開始する // distance <= 100
    data modify storage asset:spawner RequiredPlayerRange set value 48
