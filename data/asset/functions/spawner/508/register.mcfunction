...
execute unless data storage randomizer: Pos unless loaded 16 74 327 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 508
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [16, 74, 327]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [53,139]
# 一度に召喚する数 (int)
