...
execute unless data storage randomizer: Pos unless loaded 41 4 24 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 5
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [41, 4, 24]
# 体力 (int) このスポナーから召喚されたMobがN体殺されると破壊されるか
    data modify storage asset:spawner HP set value 2147483647
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [162]
# 一度に召喚する数 (int)
