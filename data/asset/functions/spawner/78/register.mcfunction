...
execute unless data storage randomizer: Pos unless loaded 188 10 -45 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 78
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [188, 10, -45]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:21,Weight:4},{Id:26,Weight:8},{Id:64,Weight:8}]
# 一度に召喚する数 (int)
