...
execute unless data storage randomizer: Pos unless loaded -197 210 -363 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 637
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-197, 210, -363]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [423]
# 一度に召喚する数 (int)
