...
execute unless data storage randomizer: Pos unless loaded 103 132 -232 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 373
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [103, 132, -232]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:138,Weight:15},{Id:413,Weight:5}]
# 一度に召喚する数 (int)
