...
execute unless data storage randomizer: Pos unless loaded 284 43 228 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 443
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [284, 43, 228]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [321]
# 一度に召喚する数 (int)
