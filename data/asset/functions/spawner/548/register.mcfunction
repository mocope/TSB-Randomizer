...
execute unless data storage randomizer: Pos unless loaded 396 26 39 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 548
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [396, 26, 39]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [141,142]
# 一度に召喚する数 (int)
