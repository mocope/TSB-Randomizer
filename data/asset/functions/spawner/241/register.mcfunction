...
execute unless data storage randomizer: Pos unless loaded -39 9 86 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 241
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-39, 9, 86]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:65,Weight:10},{Id:27,Weight:10}]
# 一度に召喚する数 (int)
