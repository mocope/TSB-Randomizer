...
execute unless data storage randomizer: Pos unless loaded 192 129 -356 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 935
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [192, 129, -356]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [187,190]
# 一度に召喚する数 (int)
