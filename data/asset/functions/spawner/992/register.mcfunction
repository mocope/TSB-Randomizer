...
execute unless data storage randomizer: Pos unless loaded 105 155 -509 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 992
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [105, 155, -509]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [189,363]
# 一度に召喚する数 (int)
