...
execute unless data storage randomizer: Pos unless loaded 89 102 1945 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 921
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [89, 102, 1945]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [150,141,187]
# 一度に召喚する数 (int)
