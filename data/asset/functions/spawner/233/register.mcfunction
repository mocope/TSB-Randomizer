...
execute unless data storage randomizer: Pos unless loaded -131 31 76 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 233
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-131, 31, 76]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [29,62,85]
# 一度に召喚する数 (int)
