...
execute unless data storage randomizer: Pos unless loaded 28 2 116 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 101
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [28, 2, 116]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [7,244]
# 一度に召喚する数 (int)
