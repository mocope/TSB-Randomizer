...
execute unless data storage randomizer: Pos unless loaded 250 137 148 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 455
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [250, 137, 148]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [234,415]
# 一度に召喚する数 (int)
