...
execute unless data storage randomizer: Pos unless loaded -59 88 -36 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 161
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-59, 88, -36]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:49,Weight:10},{Id:65,Weight:5},{Id:124,Weight:5}]
# 一度に召喚する数 (int)
