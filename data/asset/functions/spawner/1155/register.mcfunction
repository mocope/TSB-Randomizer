...
execute unless data storage randomizer: Pos unless loaded -456 309 118 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1155
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-456, 309, 118]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:417,Weight:1},{Id:420,Weight:1}]
# 一度に召喚する数 (int)
