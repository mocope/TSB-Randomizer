...
execute unless data storage randomizer: Pos unless loaded 285 137 408 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1131
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [285, 137, 408]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:187,Weight:1},{Id:201,Weight:1}]
# 一度に召喚する数 (int)
