...
execute unless data storage randomizer: Pos unless loaded -65 34 -53 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 42
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-65, 34, -53]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:63,Weight:5},{Id:31,Weight:15}]
# 一度に召喚する数 (int)
