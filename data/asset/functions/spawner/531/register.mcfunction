...
execute unless data storage randomizer: Pos unless loaded 416 92 56 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 531
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [416, 92, 56]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:232,Weight:6},{Id:142,Weight:14}]
# 一度に召喚する数 (int)
