...
execute unless data storage randomizer: Pos unless loaded 162 167 -197 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 602
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [162, 167, -197]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:190,Weight:2},{Id:371,Weight:1}]
# 一度に召喚する数 (int)
