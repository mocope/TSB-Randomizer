...
execute unless data storage randomizer: Pos unless loaded -74 161 399 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 304
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-74, 161, 399]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:273,Weight:7},{Id:274,Weight:7},{Id:292,Weight:6}]
# 一度に召喚する数 (int)
