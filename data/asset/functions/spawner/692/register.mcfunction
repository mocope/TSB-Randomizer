...
execute unless data storage randomizer: Pos unless loaded -133 170 -327 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 692
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-133, 170, -327]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:257,Weight:1},{Id:84,Weight:2}]
# 一度に召喚する数 (int)
