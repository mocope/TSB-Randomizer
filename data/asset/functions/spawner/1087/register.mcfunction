...
execute unless data storage randomizer: Pos unless loaded 107 32 549 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1087
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [107, 32, 549]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [83,330]
# 一度に召喚する数 (int)
