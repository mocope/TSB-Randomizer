...
execute unless data storage randomizer: Pos unless loaded 275 106 -275 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1146
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [275, 106, -275]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [253]
# 一度に召喚する数 (int)
