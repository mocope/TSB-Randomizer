...
execute unless data storage randomizer: Pos unless loaded -80 88 -30 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 162
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-80, 88, -30]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:56,Weight:10},{Id:65,Weight:5},{Id:124,Weight:5}]
# 一度に召喚する数 (int)
