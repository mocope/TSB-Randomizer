...
execute unless data storage randomizer: Pos unless loaded -161 187 172 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 757
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-161, 187, 172]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [160,159]
# 一度に召喚する数 (int)
