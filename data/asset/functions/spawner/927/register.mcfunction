...
execute unless data storage randomizer: Pos unless loaded 94 125 1950 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 927
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [94, 125, 1950]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:232,Weight:1},{Id:233,Weight:2},{Id:141,Weight:3},{Id:142,Weight:1}]
# 一度に召喚する数 (int)
