...
execute unless data storage randomizer: Pos unless loaded -28 28 -124 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 52
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-28, 28, -124]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:244,Weight:17},{Id:64,Weight:3}]
# 一度に召喚する数 (int)
