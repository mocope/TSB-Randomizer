...
execute unless data storage randomizer: Pos unless loaded 170 -2 76 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 202
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [170, -2, 76]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:336,Weight:8},{Id:23,Weight:12}]
# 一度に召喚する数 (int)
