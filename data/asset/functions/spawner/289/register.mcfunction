...
execute unless data storage randomizer: Pos unless loaded -59 240 386 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 289
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-59, 240, 386]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [363,369,370,371]
# 一度に召喚する数 (int)
