...
execute unless data storage randomizer: Pos unless loaded -70 241 398 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1168
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-70, 241, 398]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [181]
# 一度に召喚する数 (int)
