...
execute unless data storage randomizer: Pos unless loaded -145 100 -126 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 261
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-145, 100, -126]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [60,13]
# 一度に召喚する数 (int)
