...
execute unless data storage randomizer: Pos unless loaded -61 112 401 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 312
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-61, 112, 401]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [26,64]
# 一度に召喚する数 (int)
