...
execute unless data storage randomizer: Pos unless loaded 220 124 -49 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 282
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [220, 124, -49]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [104]
# 一度に召喚する数 (int)
