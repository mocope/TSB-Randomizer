...
execute unless data storage randomizer: Pos unless loaded -59 60 391 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 323
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-59, 60, 391]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [244,77]
# 一度に召喚する数 (int)
