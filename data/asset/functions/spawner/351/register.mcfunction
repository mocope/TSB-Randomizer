...
execute unless data storage randomizer: Pos unless loaded 82 114 -222 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 351
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [82, 114, -222]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:47,Weight:10},{Id:53,Weight:10}]
# 一度に召喚する数 (int)
