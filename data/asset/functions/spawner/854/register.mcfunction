...
execute unless data storage randomizer: Pos unless loaded 119 145 192 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 854
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [119, 145, 192]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [424]
# 一度に召喚する数 (int)
