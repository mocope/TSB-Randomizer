...
execute unless data storage randomizer: Pos unless loaded 92 157 457 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 890
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [92, 157, 457]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [150,153]
# 一度に召喚する数 (int)
