...
execute unless data storage randomizer: Pos unless loaded 123 147 -555 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 986
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [123, 147, -555]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [201,181,414]
# 一度に召喚する数 (int)
