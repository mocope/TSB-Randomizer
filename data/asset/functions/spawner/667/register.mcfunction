...
execute unless data storage randomizer: Pos unless loaded -177 178 -365 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 667
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-177, 178, -365]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [371]
# 一度に召喚する数 (int)
