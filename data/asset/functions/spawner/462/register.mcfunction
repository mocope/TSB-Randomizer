...
execute unless data storage randomizer: Pos unless loaded 303 32 -89 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 462
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [303, 32, -89]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:217,Weight:1},{Id:90,Weight:4}]
# 一度に召喚する数 (int)
