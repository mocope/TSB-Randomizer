...
execute unless data storage randomizer: Pos unless loaded 221 124 -33 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 285
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [221, 124, -33]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:104,Weight:15},{Id:138,Weight:5}]
# 一度に召喚する数 (int)
