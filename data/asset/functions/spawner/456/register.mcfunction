...
execute unless data storage randomizer: Pos unless loaded 245 139 161 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 456
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [245, 139, 161]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [182,138]
# 一度に召喚する数 (int)
