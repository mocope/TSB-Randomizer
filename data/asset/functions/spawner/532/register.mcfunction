...
execute unless data storage randomizer: Pos unless loaded 407 97 51 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 532
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [407, 97, 51]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:233,Weight:10},{Id:141,Weight:10}]
# 一度に召喚する数 (int)
