...
execute unless data storage randomizer: Pos unless loaded -203 63 393 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 779
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-203, 63, 393]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [257,370]
# 一度に召喚する数 (int)
