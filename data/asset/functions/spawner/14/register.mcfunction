...
execute unless data storage randomizer: Pos unless loaded 94 29 82 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 14
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [94, 29, 82]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:63,Weight:10},{Id:7,Weight:10}]
# 一度に召喚する数 (int)
