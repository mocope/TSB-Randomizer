...
execute unless data storage randomizer: Pos unless loaded 169 16 -55 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 86
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [169, 16, -55]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:21,Weight:5},{Id:75,Weight:10},{Id:30,Weight:1}]
# 一度に召喚する数 (int)
