...
execute unless data storage randomizer: Pos unless loaded -481 67 79 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1014
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-481, 67, 79]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [419]
# 一度に召喚する数 (int)
