...
execute unless data storage randomizer: Pos unless loaded 179 129 -351 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 936
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [179, 129, -351]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [76,190]
# 一度に召喚する数 (int)
