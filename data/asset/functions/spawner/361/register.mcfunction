...
execute unless data storage randomizer: Pos unless loaded 116 95 -254 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 361
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [116, 95, -254]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:124,Weight:5},{Id:182,Weight:10},{Id:413,Weight:5}]
# 一度に召喚する数 (int)
