...
execute unless data storage randomizer: Pos unless loaded 190 129 -371 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 941
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [190, 129, -371]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [181,190]
# 一度に召喚する数 (int)
