...
execute unless data storage randomizer: Pos unless loaded 95 15 82 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 16
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [95, 15, 82]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [63]
# 一度に召喚する数 (int)
