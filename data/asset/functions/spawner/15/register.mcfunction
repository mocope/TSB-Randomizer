...
execute unless data storage randomizer: Pos unless loaded 94 21 81 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 15
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [94, 21, 81]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:63,Weight:15},{Id:26,Weight:5}]
# 一度に召喚する数 (int)
