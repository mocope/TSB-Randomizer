...
execute unless data storage randomizer: Pos unless loaded -175 169 -377 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 700
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-175, 169, -377]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:76,Weight:3}]
# 一度に召喚する数 (int)
