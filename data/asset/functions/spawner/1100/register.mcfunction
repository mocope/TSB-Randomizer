...
execute unless data storage randomizer: Pos unless loaded 107 73 625 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1100
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [107, 73, 625]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [330,190]
# 一度に召喚する数 (int)
