...
execute unless data storage randomizer: Pos unless loaded 223 154 -47 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 277
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [223, 154, -47]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [138,47]
# 一度に召喚する数 (int)
