...
execute unless data storage randomizer: Pos unless loaded -54 69 -287 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 268
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-54, 69, -287]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [74,321]
# 一度に召喚する数 (int)
