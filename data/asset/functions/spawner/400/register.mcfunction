...
execute unless data storage randomizer: Pos unless loaded -212 81 18 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 400
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-212, 81, 18]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [261,138]
# 一度に召喚する数 (int)
