...
execute unless data storage randomizer: Pos unless loaded 154 167 -466 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 973
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [154, 167, -466]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [201,363]
# 一度に召喚する数 (int)
