...
execute unless data storage randomizer: Pos unless loaded 185 154 -372 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 951
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [185, 154, -372]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [219,220]
# 一度に召喚する数 (int)
