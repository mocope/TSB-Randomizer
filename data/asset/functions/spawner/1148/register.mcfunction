...
execute unless data storage randomizer: Pos unless loaded 86 152 -244 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1148
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [86, 152, -244]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:190,Weight:1},{Id:84,Weight:2},{Id:114,Weight:1}]
# 一度に召喚する数 (int)
