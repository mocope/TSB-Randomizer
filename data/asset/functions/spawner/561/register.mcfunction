...
execute unless data storage randomizer: Pos unless loaded 482 90 -134 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 561
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [482, 90, -134]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [148,147]
# 一度に召喚する数 (int)
