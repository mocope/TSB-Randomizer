...
execute unless data storage randomizer: Pos unless loaded -461 143 58 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1050
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-461, 143, 58]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [420]
# 一度に召喚する数 (int)
