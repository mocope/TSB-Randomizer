...
execute unless data storage randomizer: Pos unless loaded 431 16 408 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 808
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [431, 16, 408]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [187,369]
# 一度に召喚する数 (int)
