...
execute unless data storage randomizer: Pos unless loaded -233 194 682 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 629
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-233, 194, 682]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:83,Weight:10},{Id:190,Weight:10}]
# 一度に召喚する数 (int)
