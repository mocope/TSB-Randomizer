...
execute unless data storage randomizer: Pos unless loaded 105 149 498 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 893
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [105, 149, 498]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [152]
# 一度に召喚する数 (int)
