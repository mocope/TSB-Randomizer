...
execute unless data storage randomizer: Pos unless loaded 38 173 667 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 899
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [38, 173, 667]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [154,156]
# 一度に召喚する数 (int)
