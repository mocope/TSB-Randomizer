...
execute unless data storage randomizer: Pos unless loaded 101 163 324 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 876
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [101, 163, 324]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:181,Weight:3},{Id:363,Weight:1}]
# 一度に召喚する数 (int)
