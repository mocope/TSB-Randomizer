...
execute unless data storage randomizer: Pos unless loaded 136 83 1900 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 911
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [136, 83, 1900]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [292,371,414]
# 一度に召喚する数 (int)
