...
execute unless data storage randomizer: Pos unless loaded 156 -1 80 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 204
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [156, -1, 80]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:336,Weight:11},{Id:17,Weight:9}]
# 一度に召喚する数 (int)
