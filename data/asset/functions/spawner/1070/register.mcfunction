...
execute unless data storage randomizer: Pos unless loaded 118 171 -561 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1070
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [118, 171, -561]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [181,363]
# 一度に召喚する数 (int)
