...
execute unless data storage randomizer: Pos unless loaded 178 132 -370 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 943
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [178, 132, -370]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [181,187]
# 一度に召喚する数 (int)
