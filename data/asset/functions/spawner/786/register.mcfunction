...
execute unless data storage randomizer: Pos unless loaded -210 98 400 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 786
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-210, 98, 400]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [363,273,414]
# 一度に召喚する数 (int)
