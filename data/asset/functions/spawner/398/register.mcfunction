...
execute unless data storage randomizer: Pos unless loaded -205 164 4 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 398
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-205, 164, 4]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [56,261]
# 一度に召喚する数 (int)
