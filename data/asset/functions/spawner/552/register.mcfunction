...
execute unless data storage randomizer: Pos unless loaded 429 33 123 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 552
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [429, 33, 123]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [141,142,233]
# 一度に召喚する数 (int)
