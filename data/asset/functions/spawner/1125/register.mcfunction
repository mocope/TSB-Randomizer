...
execute unless data storage randomizer: Pos unless loaded 155 172 -388 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1125
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [155, 172, -388]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:189,Weight:1},{Id:190,Weight:1}]
# 一度に召喚する数 (int)
