...
execute unless data storage randomizer: Pos unless loaded 60 17 213 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 193
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [60, 17, 213]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:49,Weight:15},{Id:81,Weight:5}]
# 一度に召喚する数 (int)
