...
execute unless data storage randomizer: Pos unless loaded 46 10 -118 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 63
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [46, 10, -118]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [244]
# 一度に召喚する数 (int)
