...
execute unless data storage randomizer: Pos unless loaded 286 136 410 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1132
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [286, 136, 410]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:204,Weight:1}]
# 一度に召喚する数 (int)
...
# 初回召喚時間 (int)
    data modify storage asset:spawner Delay set value 40
# 最低召喚間隔 (int)
    data modify storage asset:spawner MinSpawnDelay set value 40
# 最大召喚間隔 (int)
    data modify storage asset:spawner MaxSpawnDelay set value 60
# 近くのエンティティの最大数 (int)
