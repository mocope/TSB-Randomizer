...
execute unless data storage randomizer: Pos unless loaded 67 86 97 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 824
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [67, 86, 97]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [83,189]
# 一度に召喚する数 (int)
