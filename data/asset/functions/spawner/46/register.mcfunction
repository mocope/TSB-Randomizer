...
execute unless data storage randomizer: Pos unless loaded -71 18 -59 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 46
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-71, 18, -59]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:239,Weight:10},{Id:31,Weight:10}]
# 一度に召喚する数 (int)
