...
execute unless data storage randomizer: Pos unless loaded 137 108 269 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 485
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [137, 108, 269]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [280,53]
# 一度に召喚する数 (int)
