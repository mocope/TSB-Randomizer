...
execute unless data storage randomizer: Pos unless loaded 127 40 -188 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 140
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [127, 40, -188]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:60,Weight:15},{Id:37,Weight:5}]
# 一度に召喚する数 (int)
