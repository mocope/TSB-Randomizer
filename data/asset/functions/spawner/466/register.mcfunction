...
execute unless data storage randomizer: Pos unless loaded 311 42 -70 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 466
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [311, 42, -70]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [53,90]
# 一度に召喚する数 (int)
