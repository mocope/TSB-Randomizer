...
execute unless data storage randomizer: Pos unless loaded -132 170 -373 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 694
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-132, 170, -373]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [83,371]
# 一度に召喚する数 (int)
