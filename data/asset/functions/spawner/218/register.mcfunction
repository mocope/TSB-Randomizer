...
execute unless data storage randomizer: Pos unless loaded 148 17 203 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 218
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [148, 17, 203]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:186,Weight:15},{Id:124,Weight:5}]
# 一度に召喚する数 (int)
