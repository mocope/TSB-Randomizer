...
execute unless data storage randomizer: Pos unless loaded -23 42 -126 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 50
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-23, 42, -126]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:115,Weight:6},{Id:191,Weight:14}]
# 一度に召喚する数 (int)
