...
execute unless data storage randomizer: Pos unless loaded 220 117 -52 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 283
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [220, 117, -52]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:271,Weight:15},{Id:139,Weight:5}]
# 一度に召喚する数 (int)
