...
execute unless data storage randomizer: Pos unless loaded -187 222 214 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 744
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-187, 222, 214]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [159,228]
# 一度に召喚する数 (int)
