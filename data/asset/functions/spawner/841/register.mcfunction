...
execute unless data storage randomizer: Pos unless loaded 148 107 92 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 841
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [148, 107, 92]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [219,414]
# 一度に召喚する数 (int)
