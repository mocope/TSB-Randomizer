...
execute unless data storage randomizer: Pos unless loaded 161 16 205 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 225
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [161, 16, 205]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:161,Weight:10},{Id:206,Weight:10}]
# 一度に召喚する数 (int)
