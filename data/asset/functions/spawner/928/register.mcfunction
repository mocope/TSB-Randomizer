...
execute unless data storage randomizer: Pos unless loaded 93 132 1948 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 928
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [93, 132, 1948]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [322,228,363]
# 一度に召喚する数 (int)
