...
execute unless data storage randomizer: Pos unless loaded -456 143 53 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1049
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-456, 143, 53]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [417]
# 一度に召喚する数 (int)
