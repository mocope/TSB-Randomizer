...
execute unless data storage randomizer: Pos unless loaded -481 113 33 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1036
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-481, 113, 33]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [83]
# 一度に召喚する数 (int)
