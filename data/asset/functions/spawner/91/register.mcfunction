...
execute unless data storage randomizer: Pos unless loaded -29 17 38 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 91
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-29, 17, 38]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [269]
# 一度に召喚する数 (int)
