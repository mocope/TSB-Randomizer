...
execute unless data storage randomizer: Pos unless loaded 15 37 -223 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 131
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [15, 37, -223]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:270,Weight:15},{Id:89,Weight:5}]
# 一度に召喚する数 (int)
