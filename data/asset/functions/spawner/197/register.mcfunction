...
execute unless data storage randomizer: Pos unless loaded 58 6 222 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 197
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [58, 6, 222]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [242,57]
# 一度に召喚する数 (int)
