...
execute unless data storage randomizer: Pos unless loaded -108 54 226 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 115
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-108, 54, 226]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [56]
# 一度に召喚する数 (int)
