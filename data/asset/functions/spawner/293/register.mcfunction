...
execute unless data storage randomizer: Pos unless loaded -66 220 402 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 293
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-66, 220, 402]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [148]
# 一度に召喚する数 (int)
