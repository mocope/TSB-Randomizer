...
execute unless data storage randomizer: Pos unless loaded 442 116 -172 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 576
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [442, 116, -172]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [204,187]
# 一度に召喚する数 (int)
