...
execute unless data storage randomizer: Pos unless loaded -59 -16 144 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 153
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-59, -16, 144]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:259,Weight:10},{Id:37,Weight:10}]
# 一度に召喚する数 (int)
