...
execute unless data storage randomizer: Pos unless loaded 130 41 -182 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 142
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [130, 41, -182]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:60,Weight:15},{Id:242,Weight:5}]
# 一度に召喚する数 (int)
