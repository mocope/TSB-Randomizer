...
execute unless data storage randomizer: Pos unless loaded -242 231 614 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 618
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-242, 231, 614]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:264,Weight:15},{Id:371,Weight:5}]
# 一度に召喚する数 (int)
