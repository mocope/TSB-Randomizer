...
execute unless data storage randomizer: Pos unless loaded -189 15 380 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 765
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-189, 15, 380]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:182,Weight:1},{Id:253,Weight:2},{Id:73,Weight:1}]
# 一度に召喚する数 (int)
