...
execute unless data storage randomizer: Pos unless loaded 246 54 242 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 449
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [246, 54, 242]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [47]
# 一度に召喚する数 (int)
