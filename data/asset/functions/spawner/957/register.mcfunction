...
execute unless data storage randomizer: Pos unless loaded 106 176 -455 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 957
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [106, 176, -455]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [363,423]
# 一度に召喚する数 (int)
