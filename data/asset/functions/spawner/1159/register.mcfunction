...
execute unless data storage randomizer: Pos unless loaded 106 223 -451 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1159
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [106, 223, -451]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [363,371]
# 一度に召喚する数 (int)
    data modify storage asset:spawner SpawnCount set value 3
# 動作範囲 (int) この範囲にプレイヤーが存在するとき、Mobの召喚を開始する
    data modify storage asset:spawner SpawnRange set value 12
# 初回召喚時間 (int)
