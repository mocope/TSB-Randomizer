...
execute unless data storage randomizer: Pos unless loaded -203 236 393 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1130
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-203, 236, 393]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [228,210]
# 一度に召喚する数 (int)
