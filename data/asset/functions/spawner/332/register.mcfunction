...
execute unless data storage randomizer: Pos unless loaded 255 111 -243 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 332
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [255, 111, -243]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [415,80]
# 一度に召喚する数 (int)
