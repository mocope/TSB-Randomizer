...
execute unless data storage randomizer: Pos unless loaded 37 131 232 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 858
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [37, 131, 232]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [84,371]
# 一度に召喚する数 (int)
