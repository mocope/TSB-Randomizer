...
execute unless data storage randomizer: Pos unless loaded 82 149 730 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 906
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [82, 149, 730]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [151,152]
# 一度に召喚する数 (int)
