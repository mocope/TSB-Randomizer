...
execute unless data storage randomizer: Pos unless loaded -198 138 1 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 396
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-198, 138, 1]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [56,80]
# 一度に召喚する数 (int)
