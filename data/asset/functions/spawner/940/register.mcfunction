...
execute unless data storage randomizer: Pos unless loaded 212 129 -374 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 940
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [212, 129, -374]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [181,189]
# 一度に召喚する数 (int)
