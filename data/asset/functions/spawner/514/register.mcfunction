...
execute unless data storage randomizer: Pos unless loaded -119 42 -25 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 514
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-119, 42, -25]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:179,Weight:15},{Id:177,Weight:5}]
# 一度に召喚する数 (int)
