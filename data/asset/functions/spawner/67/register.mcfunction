...
execute unless data storage randomizer: Pos unless loaded 82 14 -118 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 67
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [82, 14, -118]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:115,Weight:17}]
# 一度に召喚する数 (int)
