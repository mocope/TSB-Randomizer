...
execute unless data storage randomizer: Pos unless loaded 309 52 -72 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 467
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [309, 52, -72]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:90,Weight:1},{Id:217,Weight:4}]
# 一度に召喚する数 (int)
