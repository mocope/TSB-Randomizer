...
execute unless data storage randomizer: Pos unless loaded 144 -1 85 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 205
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [144, -1, 85]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:336,Weight:11},{Id:23,Weight:9}]
# 一度に召喚する数 (int)
