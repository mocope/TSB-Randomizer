...
execute unless data storage randomizer: Pos unless loaded 46 18 212 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 188
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [46, 18, 212]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:49,Weight:15},{Id:161,Weight:5}]
# 一度に召喚する数 (int)
