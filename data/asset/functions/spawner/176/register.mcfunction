...
execute unless data storage randomizer: Pos unless loaded -6 2 165 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 176
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-6, 2, 165]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [186,206]
# 一度に召喚する数 (int)
