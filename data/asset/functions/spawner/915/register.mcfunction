...
execute unless data storage randomizer: Pos unless loaded 153 92 1935 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 915
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [153, 92, 1935]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [201,204,154]
# 一度に召喚する数 (int)
