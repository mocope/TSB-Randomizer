...
execute unless data storage randomizer: Pos unless loaded -147 222 193 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 764
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-147, 222, 193]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [159,424]
# 一度に召喚する数 (int)
