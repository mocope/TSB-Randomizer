...
execute unless data storage randomizer: Pos unless loaded -183 170 -338 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 684
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-183, 170, -338]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [84,369]
# 一度に召喚する数 (int)
