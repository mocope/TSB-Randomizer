...
execute unless data storage randomizer: Pos unless loaded 162 167 -171 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 601
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [162, 167, -171]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [204,219,220]
# 一度に召喚する数 (int)
