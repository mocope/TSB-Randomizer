...
execute unless data storage randomizer: Pos unless loaded 166 108 269 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 486
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [166, 108, 269]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [182,139]
# 一度に召喚する数 (int)
