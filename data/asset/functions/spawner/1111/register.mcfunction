...
execute unless data storage randomizer: Pos unless loaded 103 207 598 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1111
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [103, 207, 598]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [273,331]
# 一度に召喚する数 (int)
