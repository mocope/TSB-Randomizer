...
execute unless data storage randomizer: Pos unless loaded -209 81 386 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 783
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-209, 81, 386]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [257,189,414]
# 一度に召喚する数 (int)
