...
execute unless data storage randomizer: Pos unless loaded -77 40 392 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 327
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-77, 40, 392]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [239]
# 一度に召喚する数 (int)
