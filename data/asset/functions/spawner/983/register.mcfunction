...
execute unless data storage randomizer: Pos unless loaded 74 150 -571 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 983
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [74, 150, -571]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [264,423,414]
# 一度に召喚する数 (int)
