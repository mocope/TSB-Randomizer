...
execute unless data storage randomizer: Pos unless loaded 279 72 -207 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 339
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [279, 72, -207]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [271,80]
# 一度に召喚する数 (int)
