...
execute unless data storage randomizer: Pos unless loaded 147 9 195 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 220
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [147, 9, 195]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:206,Weight:10},{Id:19,Weight:10}]
# 一度に召喚する数 (int)
