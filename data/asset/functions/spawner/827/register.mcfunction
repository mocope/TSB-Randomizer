...
execute unless data storage randomizer: Pos unless loaded 52 100 101 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 827
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [52, 100, 101]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [76,337]
# 一度に召喚する数 (int)
