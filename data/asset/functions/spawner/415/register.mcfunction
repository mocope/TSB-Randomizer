...
execute unless data storage randomizer: Pos unless loaded -253 98 157 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 415
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-253, 98, 157]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:171,Weight:16},{Id:217,Weight:4}]
# 一度に召喚する数 (int)
