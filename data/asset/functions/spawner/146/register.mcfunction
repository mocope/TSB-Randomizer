...
execute unless data storage randomizer: Pos unless loaded 153 82 30 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 146
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [153, 82, 30]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [236]
# 一度に召喚する数 (int)
