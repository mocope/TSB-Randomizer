...
execute unless data storage randomizer: Pos unless loaded -67 91 402 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 316
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-67, 91, 402]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [89]
# 一度に召喚する数 (int)
