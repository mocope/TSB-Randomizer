...
execute unless data storage randomizer: Pos unless loaded -282 132 -142 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 706
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-282, 132, -142]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [187,204]
# 一度に召喚する数 (int)
