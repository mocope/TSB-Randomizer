...
execute unless data storage randomizer: Pos unless loaded -233 194 546 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 622
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-233, 194, 546]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:190,Weight:12},{Id:414,Weight:8}]
# 一度に召喚する数 (int)
