...
execute unless data storage randomizer: Pos unless loaded 151 100 277 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 480
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [151, 100, 277]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [280,139]
# 一度に召喚する数 (int)
