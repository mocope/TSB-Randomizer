...
execute unless data storage randomizer: Pos unless loaded 145 4 120 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 206
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [145, 4, 120]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:336,Weight:14},{Id:17,Weight:6}]
# 一度に召喚する数 (int)
