...
execute unless data storage randomizer: Pos unless loaded -152 88 -146 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 257
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-152, 88, -146]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [259,13]
# 一度に召喚する数 (int)
