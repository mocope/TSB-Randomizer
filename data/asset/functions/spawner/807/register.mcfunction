...
execute unless data storage randomizer: Pos unless loaded 422 25 410 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 807
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [422, 25, 410]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:204,Weight:10},{Id:220,Weight:10}]
# 一度に召喚する数 (int)
