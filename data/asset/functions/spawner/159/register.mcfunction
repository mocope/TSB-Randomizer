...
execute unless data storage randomizer: Pos unless loaded -59 11 153 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 159
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-59, 11, 153]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:23,Weight:10},{Id:206,Weight:10}]
# 一度に召喚する数 (int)
