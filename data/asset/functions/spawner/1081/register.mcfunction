...
execute unless data storage randomizer: Pos unless loaded 155 19 609 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1081
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [155, 19, 609]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:330,Weight:3},{Id:80,Weight:7}]
# 一度に召喚する数 (int)
