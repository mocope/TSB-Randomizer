...
execute unless data storage randomizer: Pos unless loaded 133 126 648 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 902
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [133, 126, 648]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [152,154,155]
# 一度に召喚する数 (int)
