...
execute unless data storage randomizer: Pos unless loaded 174 14 -21 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 87
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [174, 14, -21]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:21,Weight:5},{Id:75,Weight:15},{Id:30,Weight:1}]
# 一度に召喚する数 (int)
