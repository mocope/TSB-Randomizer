...
execute unless data storage randomizer: Pos unless loaded 143 70 569 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1096
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [143, 70, 569]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [210,331]
# 一度に召喚する数 (int)
