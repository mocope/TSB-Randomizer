...
execute unless data storage randomizer: Pos unless loaded 196 112 -335 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 932
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [196, 112, -335]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [189,187]
# 一度に召喚する数 (int)
