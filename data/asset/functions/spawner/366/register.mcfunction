...
execute unless data storage randomizer: Pos unless loaded 62 121 -226 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 366
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [62, 121, -226]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [90]
# 一度に召喚する数 (int)
