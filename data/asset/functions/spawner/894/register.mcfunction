...
execute unless data storage randomizer: Pos unless loaded 119 149 498 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 894
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [119, 149, 498]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [156,154]
# 一度に召喚する数 (int)
