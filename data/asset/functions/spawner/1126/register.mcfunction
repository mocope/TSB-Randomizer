...
execute unless data storage randomizer: Pos unless loaded -218 214 408 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1126
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-218, 214, 408]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:182,Weight:3},{Id:414,Weight:1}]
# 一度に召喚する数 (int)
