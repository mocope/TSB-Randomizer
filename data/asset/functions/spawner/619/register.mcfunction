...
execute unless data storage randomizer: Pos unless loaded -223 232 577 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 619
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-223, 232, 577]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:83,Weight:15},{Id:363,Weight:5}]
# 一度に召喚する数 (int)
