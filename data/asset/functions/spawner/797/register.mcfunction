...
execute unless data storage randomizer: Pos unless loaded 294 22 408 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 797
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [294, 22, 408]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [187,287]
# 一度に召喚する数 (int)
