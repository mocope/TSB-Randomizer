...
execute unless data storage randomizer: Pos unless loaded -285 151 -177 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 710
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-285, 151, -177]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [287,201]
# 一度に召喚する数 (int)
