...
execute unless data storage randomizer: Pos unless loaded 222 7 129 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 213
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [222, 7, 129]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:336,Weight:17},{Id:202,Weight:3}]
# 一度に召喚する数 (int)
