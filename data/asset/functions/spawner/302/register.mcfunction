...
execute unless data storage randomizer: Pos unless loaded -68 171 403 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 302
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-68, 171, 403]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:107,Weight:10},{Id:413,Weight:10}]
# 一度に召喚する数 (int)
