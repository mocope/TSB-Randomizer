...
execute unless data storage randomizer: Pos unless loaded 78 161 -447 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 978
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [78, 161, -447]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [264,423]
# 一度に召喚する数 (int)
