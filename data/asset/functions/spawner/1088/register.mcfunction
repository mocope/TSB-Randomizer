...
execute unless data storage randomizer: Pos unless loaded 65 32 591 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1088
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [65, 32, 591]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [210,330]
# 一度に召喚する数 (int)
