...
execute unless data storage randomizer: Pos unless loaded 280 74 -237 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 336
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [280, 74, -237]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [107,80]
# 一度に召喚する数 (int)
