...
execute unless data storage randomizer: Pos unless loaded -286 227 -178 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 715
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-286, 227, -178]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [204,182]
# 一度に召喚する数 (int)
