...
execute unless data storage randomizer: Pos unless loaded -30 2 107 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 239
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-30, 2, 107]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:57,Weight:15},{Id:19,Weight:5}]
# 一度に召喚する数 (int)
