...
execute unless data storage randomizer: Pos unless loaded -162 235 -349 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1157
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-162, 235, -349]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:190,Weight:1},{Id:114,Weight:1}]
# 一度に召喚する数 (int)
