...
execute unless data storage randomizer: Pos unless loaded 187 -3 83 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 200
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [187, -3, 83]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:336,Weight:8},{Id:202,Weight:12}]
# 一度に召喚する数 (int)
