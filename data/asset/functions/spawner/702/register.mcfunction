...
execute unless data storage randomizer: Pos unless loaded -191 170 -371 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 702
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-191, 170, -371]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:83,Weight:1},{Id:257,Weight:2}]
# 一度に召喚する数 (int)
