...
execute unless data storage randomizer: Pos unless loaded -172 205 587 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 611
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-172, 205, 587]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:190,Weight:15},{Id:371,Weight:5}]
# 一度に召喚する数 (int)
