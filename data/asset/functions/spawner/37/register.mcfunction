...
execute unless data storage randomizer: Pos unless loaded -83 17 6 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 37
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-83, 17, 6]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:238,Weight:15}]
# 一度に召喚する数 (int)
