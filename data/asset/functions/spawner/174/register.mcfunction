...
execute unless data storage randomizer: Pos unless loaded 200 15 -131 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 174
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [200, 15, -131]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:227,Weight:10},{Id:49,Weight:10}]
# 一度に召喚する数 (int)
