...
execute unless data storage randomizer: Pos unless loaded -143 169 -347 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 696
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-143, 169, -347]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [190,371]
# 一度に召喚する数 (int)
