...
execute unless data storage randomizer: Pos unless loaded -217 204 614 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 609
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-217, 204, 614]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:83,Weight:12},{Id:264,Weight:8}]
# 一度に召喚する数 (int)
