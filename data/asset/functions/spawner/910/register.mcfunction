...
execute unless data storage randomizer: Pos unless loaded 121 83 1907 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 910
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [121, 83, 1907]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [153,83,233]
# 一度に召喚する数 (int)
