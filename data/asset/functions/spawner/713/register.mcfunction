...
execute unless data storage randomizer: Pos unless loaded -283 184 -171 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 713
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-283, 184, -171]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [182,204]
# 一度に召喚する数 (int)
