...
execute unless data storage randomizer: Pos unless loaded -235 83 178 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 410
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-235, 83, 178]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:171,Weight:10},{Id:177,Weight:10}]
# 一度に召喚する数 (int)
