...
execute unless data storage randomizer: Pos unless loaded 450 9 408 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 809
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [450, 9, 408]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:204,Weight:10},{Id:369,Weight:10}]
# 一度に召喚する数 (int)
