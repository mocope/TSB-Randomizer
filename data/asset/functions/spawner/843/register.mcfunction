...
execute unless data storage randomizer: Pos unless loaded 114 124 182 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 843
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [114, 124, 182]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [160,370]
# 一度に召喚する数 (int)
