...
execute unless data storage randomizer: Pos unless loaded 149 91 1902 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 913
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [149, 91, 1902]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [187,189,190]
# 一度に召喚する数 (int)
