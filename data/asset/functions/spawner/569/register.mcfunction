...
execute unless data storage randomizer: Pos unless loaded 444 80 -175 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 569
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [444, 80, -175]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [369,219]
# 一度に召喚する数 (int)
