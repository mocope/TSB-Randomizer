...
execute unless data storage randomizer: Pos unless loaded -173 222 158 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 762
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-173, 222, 158]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [424,370]
# 一度に召喚する数 (int)
