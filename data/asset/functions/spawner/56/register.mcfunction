...
execute unless data storage randomizer: Pos unless loaded -60 42 -164 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 56
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-60, 42, -164]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [26]
# 一度に召喚する数 (int)
