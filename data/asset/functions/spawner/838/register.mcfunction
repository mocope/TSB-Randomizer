...
execute unless data storage randomizer: Pos unless loaded 166 100 89 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 838
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [166, 100, 89]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [220,414]
# 一度に召喚する数 (int)
