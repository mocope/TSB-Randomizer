...
execute unless data storage randomizer: Pos unless loaded -461 67 69 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1015
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-461, 67, 69]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [16,322]
# 一度に召喚する数 (int)
