...
execute unless data storage randomizer: Pos unless loaded 161 19 570 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1079
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [161, 19, 570]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [330,331]
# 一度に召喚する数 (int)
