...
execute unless data storage randomizer: Pos unless loaded 47 181 612 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 898
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [47, 181, 612]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [155,153]
# 一度に召喚する数 (int)
