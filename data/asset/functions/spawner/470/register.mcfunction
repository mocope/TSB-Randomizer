...
execute unless data storage randomizer: Pos unless loaded 301 54 -80 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 470
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [301, 54, -80]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [90,47]
# 一度に召喚する数 (int)
