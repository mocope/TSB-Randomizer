...
execute unless data storage randomizer: Pos unless loaded -138 78 143 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 406
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-138, 78, 143]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [186,73]
# 一度に召喚する数 (int)
