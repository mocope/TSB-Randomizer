...
execute unless data storage randomizer: Pos unless loaded 201 127 -355 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 934
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [201, 127, -355]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [187,187]
# 一度に召喚する数 (int)
