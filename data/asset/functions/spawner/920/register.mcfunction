...
execute unless data storage randomizer: Pos unless loaded 104 102 1932 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 920
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [104, 102, 1932]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [274,330,331]
# 一度に召喚する数 (int)
