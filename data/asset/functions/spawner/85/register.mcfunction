...
execute unless data storage randomizer: Pos unless loaded 211 12 -51 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 85
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [211, 12, -51]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:21,Weight:8},{Id:26,Weight:6},{Id:64,Weight:1}]
# 一度に召喚する数 (int)
