...
execute unless data storage randomizer: Pos unless loaded -20 2 170 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 179
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-20, 2, 170]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:19,Weight:1},{Id:259,Weight:4}]
# 一度に召喚する数 (int)
