...
execute unless data storage randomizer: Pos unless loaded 291 32 -80 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 459
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [291, 32, -80]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:217,Weight:1},{Id:53,Weight:4}]
# 一度に召喚する数 (int)
