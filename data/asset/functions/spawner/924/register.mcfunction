...
execute unless data storage randomizer: Pos unless loaded 105 109 1945 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 924
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [105, 109, 1945]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [417,418,419]
# 一度に召喚する数 (int)
