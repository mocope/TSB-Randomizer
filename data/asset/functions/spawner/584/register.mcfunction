...
execute unless data storage randomizer: Pos unless loaded 471 216 -146 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 584
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [471, 216, -146]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [220]
# 一度に召喚する数 (int)
