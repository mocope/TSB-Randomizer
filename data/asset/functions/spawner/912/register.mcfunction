...
execute unless data storage randomizer: Pos unless loaded 137 92 1914 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 912
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [137, 92, 1914]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [414,233,151]
# 一度に召喚する数 (int)
