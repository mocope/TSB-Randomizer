...
execute unless data storage randomizer: Pos unless loaded 25 117 -378 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1124
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [25, 117, -378]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:146,Weight:3},{Id:370,Weight:1}]
# 一度に召喚する数 (int)
