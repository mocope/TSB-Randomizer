...
execute unless data storage randomizer: Pos unless loaded -275 53 183 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 420
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-275, 53, 183]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:217,Weight:4},{Id:259,Weight:16}]
# 一度に召喚する数 (int)
