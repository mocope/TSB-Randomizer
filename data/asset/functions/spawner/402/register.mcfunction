...
execute unless data storage randomizer: Pos unless loaded -135 111 -231 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 402
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-135, 111, -231]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:47,Weight:1},{Id:53,Weight:1},{Id:80,Weight:1}]
# 一度に召喚する数 (int)
