...
execute unless data storage randomizer: Pos unless loaded -174 266 186 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1164
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-174, 266, 186]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:228,Weight:1},{Id:370,Weight:1}]
# 一度に召喚する数 (int)
