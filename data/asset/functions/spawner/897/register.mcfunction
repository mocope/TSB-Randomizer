...
execute unless data storage randomizer: Pos unless loaded 97 179 644 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 897
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [97, 179, 644]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [150]
# 一度に召喚する数 (int)
