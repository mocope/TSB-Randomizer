...
execute unless data storage randomizer: Pos unless loaded 134 16 43 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 19
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [134, 16, 43]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:63,Weight:15},{Id:58,Weight:5}]
# 一度に召喚する数 (int)
