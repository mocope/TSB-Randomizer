...
execute unless data storage randomizer: Pos unless loaded 202 13 -43 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 88
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [202, 13, -43]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:21,Weight:5},{Id:75,Weight:15}]
# 一度に召喚する数 (int)
