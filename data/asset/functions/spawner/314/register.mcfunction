...
execute unless data storage randomizer: Pos unless loaded -59 100 397 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 314
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-59, 100, 397]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [202]
# 一度に召喚する数 (int)
