...
execute unless data storage randomizer: Pos unless loaded 216 12 -61 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 80
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [216, 12, -61]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:21,Weight:10},{Id:26,Weight:5},{Id:64,Weight:5}]
# 一度に召喚する数 (int)
