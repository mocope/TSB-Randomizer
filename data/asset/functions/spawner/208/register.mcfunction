...
execute unless data storage randomizer: Pos unless loaded 164 2 122 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 208
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [164, 2, 122]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:336,Weight:14},{Id:245,Weight:6}]
# 一度に召喚する数 (int)
