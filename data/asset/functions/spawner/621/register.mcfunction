...
execute unless data storage randomizer: Pos unless loaded -202 235 576 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 621
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-202, 235, 576]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:190,Weight:8},{Id:264,Weight:4},{Id:181,Weight:8}]
# 一度に召喚する数 (int)
