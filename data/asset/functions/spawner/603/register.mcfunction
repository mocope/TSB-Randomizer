...
execute unless data storage randomizer: Pos unless loaded 186 166 -196 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 603
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [186, 166, -196]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:146,Weight:1},{Id:147,Weight:1},{Id:322,Weight:1}]
# 一度に召喚する数 (int)
