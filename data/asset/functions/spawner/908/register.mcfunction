...
execute unless data storage randomizer: Pos unless loaded 108 83 1883 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 908
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [108, 83, 1883]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [414,201,371]
# 一度に召喚する数 (int)
