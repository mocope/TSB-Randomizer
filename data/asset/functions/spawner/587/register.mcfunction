...
execute unless data storage randomizer: Pos unless loaded 475 216 -162 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 587
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [475, 216, -162]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [210,337]
# 一度に召喚する数 (int)
