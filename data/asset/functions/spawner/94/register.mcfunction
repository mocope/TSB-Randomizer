...
execute unless data storage randomizer: Pos unless loaded 81 -12 47 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 94
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [81, -12, 47]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [29]
# 一度に召喚する数 (int)
