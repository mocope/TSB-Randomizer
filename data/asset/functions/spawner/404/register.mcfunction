...
execute unless data storage randomizer: Pos unless loaded -130 73 144 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 404
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-130, 73, 144]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [186,253]
# 一度に召喚する数 (int)
