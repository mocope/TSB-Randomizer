...
execute unless data storage randomizer: Pos unless loaded 189 155 -194 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 605
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [189, 155, -194]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [292,274,273]
# 一度に召喚する数 (int)
