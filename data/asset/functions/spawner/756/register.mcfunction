...
execute unless data storage randomizer: Pos unless loaded -158 187 155 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 756
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-158, 187, 155]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [159,160]
# 一度に召喚する数 (int)
