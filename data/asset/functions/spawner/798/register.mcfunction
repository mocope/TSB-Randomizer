...
execute unless data storage randomizer: Pos unless loaded 324 17 405 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 798
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [324, 17, 405]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:287,Weight:15},{Id:369,Weight:5}]
# 一度に召喚する数 (int)
