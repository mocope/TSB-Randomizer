...
execute unless data storage randomizer: Pos unless loaded 101 116 189 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 849
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [101, 116, 189]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [424,148]
# 一度に召喚する数 (int)
