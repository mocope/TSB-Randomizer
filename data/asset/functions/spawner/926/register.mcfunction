...
execute unless data storage randomizer: Pos unless loaded 98 117 1959 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 926
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [98, 117, 1959]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [330,147,257]
# 一度に召喚する数 (int)
