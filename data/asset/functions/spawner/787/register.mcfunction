...
execute unless data storage randomizer: Pos unless loaded -190 107 398 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 787
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-190, 107, 398]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [190,189]
# 一度に召喚する数 (int)
