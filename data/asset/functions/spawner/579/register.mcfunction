...
execute unless data storage randomizer: Pos unless loaded 462 181 -145 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 579
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [462, 181, -145]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [114,228]
# 一度に召喚する数 (int)
