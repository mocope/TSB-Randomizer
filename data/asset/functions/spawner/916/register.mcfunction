...
execute unless data storage randomizer: Pos unless loaded 162 93 1963 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 916
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [162, 93, 1963]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [414,156,84]
# 一度に召喚する数 (int)
