...
execute unless data storage randomizer: Pos unless loaded 442 15 112 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 543
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [442, 15, 112]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [142]
# 一度に召喚する数 (int)
