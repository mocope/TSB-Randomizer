...
execute unless data storage randomizer: Pos unless loaded -130 31 67 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 231
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-130, 31, 67]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [5,23]
# 一度に召喚する数 (int)
