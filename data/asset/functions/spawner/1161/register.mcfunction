...
execute unless data storage randomizer: Pos unless loaded 442 148 -172 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1161
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [442, 148, -172]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:369,Weight:1},{Id:337,Weight:1}]
# 一度に召喚する数 (int)
