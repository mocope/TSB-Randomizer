...
execute unless data storage randomizer: Pos unless loaded 86 17 135 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 9
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [86, 17, 135]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:115,Weight:10},{Id:191,Weight:10}]
# 一度に召喚する数 (int)
