...
execute unless data storage randomizer: Pos unless loaded -11 18 180 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 182
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-11, 18, 180]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:19,Weight:1},{Id:206,Weight:4}]
# 一度に召喚する数 (int)
