...
execute unless data storage randomizer: Pos unless loaded 174 203 -185 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 607
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [174, 203, -185]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [322]
# 一度に召喚する数 (int)
