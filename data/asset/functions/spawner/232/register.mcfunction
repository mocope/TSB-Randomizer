...
execute unless data storage randomizer: Pos unless loaded -145 31 67 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 232
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-145, 31, 67]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [29,62]
# 一度に召喚する数 (int)
