...
execute unless data storage randomizer: Pos unless loaded -279 151 -171 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 711
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-279, 151, -171]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [182,201]
# 一度に召喚する数 (int)
