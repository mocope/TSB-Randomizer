...
execute unless data storage randomizer: Pos unless loaded -202 248 614 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1152
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-202, 248, 614]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:363,Weight:1},{Id:371,Weight:1}]
# 一度に召喚する数 (int)
