...
execute unless data storage randomizer: Pos unless loaded 121 34 40 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 22
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [121, 34, 40]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:85,Weight:10},{Id:77,Weight:10}]
# 一度に召喚する数 (int)
