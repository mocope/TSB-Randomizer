...
execute unless data storage randomizer: Pos unless loaded 121 91 1977 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 918
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [121, 91, 1977]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [424,287,292]
# 一度に召喚する数 (int)
