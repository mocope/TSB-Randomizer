...
execute unless data storage randomizer: Pos unless loaded 112 132 168 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 847
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [112, 132, 168]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [424,146]
# 一度に召喚する数 (int)
