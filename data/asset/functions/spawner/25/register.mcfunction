...
execute unless data storage randomizer: Pos unless loaded -71 18 49 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 25
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-71, 18, 49]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:21,Weight:13},{Id:38,Weight:7}]
# 一度に召喚する数 (int)
