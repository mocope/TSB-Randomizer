...
execute unless data storage randomizer: Pos unless loaded 108 128 659 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 903
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [108, 128, 659]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [154]
# 一度に召喚する数 (int)
