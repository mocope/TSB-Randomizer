...
execute unless data storage randomizer: Pos unless loaded 452 216 -145 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 585
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [452, 216, -145]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [337]
# 一度に召喚する数 (int)
