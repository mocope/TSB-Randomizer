...
execute unless data storage randomizer: Pos unless loaded 66 142 490 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 892
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [66, 142, 490]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [151,156,150]
# 一度に召喚する数 (int)
