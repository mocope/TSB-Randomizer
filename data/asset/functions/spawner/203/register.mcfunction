...
execute unless data storage randomizer: Pos unless loaded 160 -2 97 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 203
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [160, -2, 97]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:336,Weight:11},{Id:202,Weight:9}]
# 一度に召喚する数 (int)
