...
execute unless data storage randomizer: Pos unless loaded -31 42 -128 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 51
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-31, 42, -128]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:115,Weight:14},{Id:191,Weight:6}]
# 一度に召喚する数 (int)
