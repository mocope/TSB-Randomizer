...
execute unless data storage randomizer: Pos unless loaded -203 135 384 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 791
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-203, 135, 384]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [257,273,363]
# 一度に召喚する数 (int)
