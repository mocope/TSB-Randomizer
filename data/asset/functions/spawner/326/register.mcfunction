...
execute unless data storage randomizer: Pos unless loaded -57 40 394 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 326
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-57, 40, 394]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [238]
# 一度に召喚する数 (int)
