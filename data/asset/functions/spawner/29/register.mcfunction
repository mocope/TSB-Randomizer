...
execute unless data storage randomizer: Pos unless loaded -85 6 46 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 29
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-85, 6, 46]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:21,Weight:17},{Id:38,Weight:3}]
# 一度に召喚する数 (int)
