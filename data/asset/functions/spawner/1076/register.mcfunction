...
execute unless data storage randomizer: Pos unless loaded 127 19 540 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1076
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [127, 19, 540]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [331,190]
# 一度に召喚する数 (int)
