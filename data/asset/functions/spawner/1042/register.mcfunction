...
execute unless data storage randomizer: Pos unless loaded -431 131 83 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1042
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-431, 131, 83]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [287]
# 一度に召喚する数 (int)
