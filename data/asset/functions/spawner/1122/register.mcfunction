...
execute unless data storage randomizer: Pos unless loaded 24 119 -393 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 1122
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [24, 119, -393]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [146,147]
# 一度に召喚する数 (int)
