...
execute unless data storage randomizer: Pos unless loaded -203 11 393 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 793
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-203, 11, 393]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [414,228,190]
# 一度に召喚する数 (int)
