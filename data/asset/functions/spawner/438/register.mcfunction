...
execute unless data storage randomizer: Pos unless loaded -64 156 174 run return 1

# ID (int)
    data modify storage asset:spawner ID set value 438
# Pos ([int] @ 3)
    data modify storage asset:spawner Pos set value [-64, 156, 174]
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:74,Weight:10},{Id:235,Weight:10}]
# 一度に召喚する数 (int)
